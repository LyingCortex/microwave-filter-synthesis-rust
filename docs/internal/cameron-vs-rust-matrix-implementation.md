...
- `transform_matrix(..., TopologyKind::Folded)` reaches the folded reduction path
- the folded reduction performs nested elimination loops over source-side and load-side
  entries
...
- `transform_matrix(..., TopologyKind::Arrow)` reaches the arrow reduction path
- the arrow reduction iteratively annihilates entries through
