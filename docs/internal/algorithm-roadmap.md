...
Run the literature JSON comparison example and regenerate the report:

```powershell
cargo run --example filter_database_case
```

Run the literature fixture showcase example:

```powershell
cargo run --example literature_fixtures
```

Run the quickstart prelude example:

```powershell
cargo run --example quickstart_prelude
```

This prints a compact formatted summary in the terminal.

Run the detailed quickstart report example:

```powershell
cargo run --example quickstart_report
```

This writes `docs/quickstart_report_output.md`.

Run the band-pass example:
...
Run only the literature-related tests:

```powershell
cargo test literature_fixtures -- --nocapture
```

Run only the JSON-driven end-to-end literature test:

```powershell
cargo test filter_database_case_can_drive_end_to_end_synthesis_flow -- --nocapture
```
...
Check all targets without executing tests:

```powershell
cargo check --all-targets
```

The literature comparison report generated by `filter_database_case` is written to:

[docs/filter_database_case_output.md](/abs/path/c:/Users/eynulai/Downloads/mfs/docs/filter_database_case_output.md)
