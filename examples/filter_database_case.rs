...
use mfs::fixtures::{ComplexValue, load_filter_database_case_from_repo, load_filter_database_end_to_end_fixture};
...
fn main() -> mfs::Result<()> {
    ...
    println!("case id: {}", fixture.case_id);
    println!("order: {}", fixture.spec.order);
    println!("return loss (dB): {}", fixture.spec.return_loss_db());
    println!(
        "normalized transmission zeros: {:?}",
        outcome.polynomials.transmission_zeros_normalized
    );
    println!(
        "band-pass mapping: center={} Hz, bandwidth={} Hz",
    ...
    println!("response samples: {}", outcome.response.samples.len());
    println!("eps: {}", outcome.polynomials.eps);
    println!("eps_r: {}", outcome.polynomials.eps_r);
    println!(
        "e coefficients: {}",
        format_complex_values(&complex_coefficients_to_values(&outcome.polynomials.e.coefficients))
    );
    println!(
        "f coefficients: {}",
        format_complex_values(&complex_coefficients_to_values(&outcome.polynomials.f.coefficients))
    );
    println!(
        "p coefficients: {}",
        format_complex_values(&complex_coefficients_to_values(&outcome.polynomials.p.coefficients))
    );
    println!("reference E length: {}", reference_model.polynomial_coefficients.e.len());
    println!("reference F length: {}", reference_model.polynomial_coefficients.f.len());
    println!("reference P length: {}", reference_model.polynomial_coefficients.p.len());
    println!("actual e_s length: {}", generalized.e_s.as_ref().map_or(0, |poly| poly.coefficients.len()));
    println!("actual f_s length: {}", generalized.f_s.coefficients.len());
    println!("actual p_s length: {}", generalized.p_s.coefficients.len());
    ...
    let actual_e = generalized
        .e_s
        .as_ref()
        .map(|poly| format_complex_values(&complex_coefficients_to_values(&poly.coefficients)))
        .unwrap_or_else(|| "None".to_string());
    let actual_f = format_complex_values(&complex_coefficients_to_values(&generalized.f_s.coefficients));
    let actual_p = format_complex_values(&complex_coefficients_to_values(&generalized.p_s.coefficients));
    ...
    let actual_e_roots = generalized
        .e_stage
        .as_ref()
        .map(|stage| format_complex_values(&complex_coefficients_to_values(&stage.e_s_roots)))
        .unwrap_or_else(|| "None".to_string());
    let actual_f_roots = generalized
        .f_s
        .roots()?
        .into_iter()
        ...
        .collect::<Vec<_>>();
    let actual_f_roots = format_complex_values(&sort_complex_values(&actual_f_roots));
    let actual_a_roots = generalized
        .a_stage
        .as_ref()
        .map(|stage| format_complex_values(&complex_coefficients_to_values(&stage.a_s_roots)))
        .unwrap_or_else(|| "None".to_string());
    let actual_raw_w_roots = generalized
        .e_stage
        .as_ref()
        .map(|stage| format_complex_values(&complex_coefficients_to_values(&stage.raw_roots)))
        .unwrap_or_else(|| "None".to_string());
    let actual_reflected_w_roots = generalized
        .e_stage
        .as_ref()
        .map(|stage| format_complex_values(&complex_coefficients_to_values(&stage.reflected_roots)))
        .unwrap_or_else(|| "None".to_string());
    ...
    let f_comparison = format_comparison_table(
        "F",
        &reference_model.polynomial_coefficients.f,
        complex_coefficients_to_values(&generalized.f_s.coefficients),
    );
    let p_comparison = format_comparison_table(
        "P",
        &reference_model.polynomial_coefficients.p,
        complex_coefficients_to_values(&generalized.p_s.coefficients),
    );
...
```\n",
        ...
        outcome.matrix_method,
        outcome.matrix.order(),
        outcome.response.samples.len(),
        outcome.polynomials.eps,
        outcome.polynomials.eps_r,
        format_complex_values(&complex_coefficients_to_values(&outcome.polynomials.e.coefficients)),
        format_complex_values(&complex_coefficients_to_values(&outcome.polynomials.f.coefficients)),
        format_complex_values(&complex_coefficients_to_values(&outcome.polynomials.p.coefficients)),
        reference_e,
