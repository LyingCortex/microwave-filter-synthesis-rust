...
/// Defines one transmission zero in normalized low-pass prototype coordinates.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct TransmissionZero {
    /// Numeric value of the zero in normalized low-pass prototype coordinates.
    ///
    /// `f64::INFINITY` represents a zero at infinity.
    pub value: f64,
}
...
impl TransmissionZero {
    ...
    /// Creates a transmission zero at infinity.
...
pub struct NormalizedFilterSpec {
    ...
    /// Transmission zeros expressed in normalized prototype coordinates.
    ///
...
mod tests {
    ...
    #[test]
    fn spec_can_store_infinite_transmission_zeros() -> Result<()> {
        let spec = NormalizedFilterSpec::new(4, 20.0)?.with_transmission_zeros([
            TransmissionZero::infinite(),
            TransmissionZero::normalized(-1.5),
        ]);

        assert!(spec.transmission_zeros[0].value.is_infinite());
        assert_eq!(spec.transmission_zeros[1].value, -1.5);
        Ok(())
    }
