use super::helpers::GeneralizedChebyshevData;
...
pub struct PolynomialSet {
    ...
    pub eps_r: f64,
    /// Transmission zeros expressed in normalized coordinates.
    ///
    /// `f64::INFINITY` represents a prescribed zero at infinity.
...
impl PolynomialSet {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        {
            return Err(MfsError::PreconditionViolation(
                "polynomial coefficients must be finite and transmission zeros must be real values or infinity".to_string(),
            ));
        }
...
mod tests {
    use super::*;
    use crate::approx::helpers::GeneralizedChebyshevData;
    use crate::approx::{ComplexCoefficient, ComplexPolynomial};
    ...
    #[test]
    fn polynomial_set_allows_infinite_transmission_zeros() {
        let polynomials = PolynomialSet::new(
            3,
            0.1,
            0.1,
        ...
        .expect("infinite transmission zeros should be accepted");

        assert!(polynomials.transmission_zeros_normalized.iter().all(|value| value.is_infinite()));
    }
}
