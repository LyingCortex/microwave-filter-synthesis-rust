...
use super::generalized_ops::{
    complex_from_real, descending_w_to_ascending_s, reciprocal_or_zero,
    reflect_to_upper_half_plane, rotate_w_root_to_s, s_to_w_coefficients, safe_sqrt_term,
    w_to_s_coefficients,
};
use super::{ComplexCoefficient, ComplexPolynomial, AdaptiveRootSolver};
...
pub fn pad_transmission_zeros(
    ...
    let mut padded = Vec::with_capacity(order);
...
pub fn cameron_recursive(padded_zeros: &[f64]) -> Result<CameronRecurrence> {
    ...
    let max_u_len = padded_zeros.len() + 1;
    let max_v_len = padded_zeros.len();
    let mut u = Vec::with_capacity(max_u_len);
    let mut v = Vec::with_capacity(max_v_len);
    u.push(1.0);
    u.push(-reciprocal_or_zero(first_zero));
    v.push(safe_sqrt_term(first_zero)?);
    let mut next_u = Vec::with_capacity(max_u_len);
    let mut next_v = Vec::with_capacity(max_v_len);
    ...
    for &next_zero in padded_zeros.iter().skip(1) {
        ...
        let reciprocal = reciprocal_or_zero(next_zero);
        let temp = safe_sqrt_term(next_zero)?;

        // This specialized update avoids the temporary convolutions and scaled
        // vectors used by the generic helpers while preserving the same
        // descending-power Cameron recurrence.
        next_u.clear();
        next_u.resize(u.len() + 1, 0.0);
        for (index, value) in u.iter().copied().enumerate() {
            next_u[index] += value;
            next_u[index + 1] -= value * reciprocal;
        }
        for (index, value) in v.iter().copied().enumerate() {
            let scaled = value * temp;
            next_u[index] += scaled;
            next_u[index + 2] -= scaled;
        }

        next_v.clear();
        next_v.resize(u.len(), 0.0);
        for (index, value) in v.iter().copied().enumerate() {
            next_v[index] += value;
            next_v[index + 1] -= value * reciprocal;
        }
        for (index, value) in u.iter().copied().enumerate() {
            next_v[index] += value * temp;
        }

        mem::swap(&mut u, &mut next_u);
        mem::swap(&mut v, &mut next_v);
    }
...
pub fn find_a_polynomial(
    ...
    Ok((
        stage
            .as_ref()
            .map(|stage| ComplexPolynomial::new(w_to_s_coefficients(&stage.a_w.coefficients)))
            .transpose()?,
        stage.map_or_else(
